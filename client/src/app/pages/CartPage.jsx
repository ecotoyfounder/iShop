...
const CartPage = () => {
    ...
    return (newCart.length > 0 ?
        <div>
            ...
            {newCart.map(item =>
                <div key={item._id}
                     className="flex justify-between rounded-xl relative bg-bgDark bg-opacity-30 m-3 p-2 text-darkColor">
                    <div className="flex justify-start">
                        <img src={item.image} alt="image" className="rounded-md mr-5" width="50px" height="50px"/>
                        <h1 className="font-bold tex-md pt-3">{item.name}</h1>
                    </div>
                    <button
                        className="bg-bgLight text-3xl font-semibold bottom-3 pb-10 rounded-md w-9 h-9 absolute ml-marginLeftX3"
                        onClick={() => {
                            handleAdd(item);
                        }}>+
                    </button>
                    <h1 className="absolute ml-marginLeftXX font-semibold pt-2.5 text-xl text-center w-8 h-8 rounded-md">{item.count}</h1>
                    <button
                        className="bg-bgLight text-3xl font-semibold bottom-3 pb-10 rounded-md w-9 h-9 absolute  ml-marginLeftXX3"
                        onClick={() => {
                            handleRemoveOneGood(item._id);
                        }}
                    >-
                    </button>
                    <button className="absolute ml-marginLeftXXX pt-3" onClick={() => handleRemoveFromCart(item._id)}
                    >
                    ...
                    <div className="font-semibold pt-3">{Math.floor(item.totalprice)}€</div>
                </div>
            )}
